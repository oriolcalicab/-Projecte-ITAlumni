"use strict";
function carregarFeinesPortal() {
    const dadesLocal = localStorage.getItem('feines');
    let feinaDeLaBaseDades = dadesLocal ? JSON.parse(dadesLocal) : [];
...
function renderitzarFeines(llistaFeines) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- CERCADOR ---
    const cercador = document.getElementById('search-job');
    if (cercador) {
    ...
    //  FILTRES 
    const enllacosFiltres = document.querySelectorAll('.filter-link');
    enllacosFiltres.forEach(enllaco => {
        enllaco.addEventListener('click', (event) => {
            ...
            const textFiltre = (_a = enllaco.textContent) === null || _a === void 0 ? void 0 : _a.trim();
...
//# sourceMappingURL=jobPortal.js.map